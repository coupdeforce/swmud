...
void do_compare_obj_to_obj(object thing1, object thing2)
{
   ...
   if (this_body->test_skill("compare", (this_body->query_guild_level("smuggler") * 10)))
   {
...
void compare(object thing1, object thing2)
{
   ...
   int spec = this_body->query_guild_specialization_rank("smuggler", "something");
   int rank_spec;
   int resolution;

   if (spec <= 0) { spec = this_body->query_guild_specialization_rank("merchant", "something"); }

   rank_spec = (rank + spec) < 0 ? 0 : (rank + spec);
   resolution = level + rank_spec;

//   write("You compare " + thing1->the_short() + " to " + thing2->the_short() + " with a resolution of " + resolution + ".\n");

   if (this_body->query_guild_level("smuggler"))
   {
      if (thing1->is_weapon() && thing2->is_weapon())
      {
         switch ((thing1->query_weapon_class() - thing2->query_weapon_class()) * resolution / 100)
         {
      ...
      {
         switch ((thing1->query_armor_class() - thing2->query_armor_class()) * resolution / 100)
         {
      ...
      {
         switch ((thing1->query_max_durability() - thing2->query_max_durability()) * resolution / 100)
         {
   ...
   {
      switch (to_int(((thing1->query_weight() * 100) - (thing2->query_weight() * 100)) * resolution / 100))
      {
      ...
      switch ((thing1->query_value() - thing2->query_value()) * resolution / 100)
      {
      ...
      {
         switch ((thing1->query_capacity() - thing2->query_capacity()) * resolution / 100)
         {
