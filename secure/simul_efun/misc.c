...
object this_body();
private nosave string array normal_directions = ({ "up", "down", "north", "northeast",
   "northwest", "east", "southeast", "southwest", "south", "west" });
...
mixed choice(mixed f)
{
   ...
   switch(typeof(f))
   {
      ...
      default: error("choice of non-sequential type");
   }
}
