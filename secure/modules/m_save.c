// Last edited by deforce on 07-11-2008
// Inheritable for persistance.
// Beek for nightmare, ported by Rust.
// Could use helper funs that save directly to a file,
// and some security (when programatic security arrives)
...
private nosave mixed array saved = ({ });
...
protected void add_save(mixed array vars)
{
   saved += vars;
...
varargs string save_to_string(int recursep)
{
   ...
   if (save_recurse)
   {
      ...
      {
         map["#inventory#"] = filter_array(all_inventory(), (: $1->is_autoload() :))->save_to_string(1) - ({ 0 });
      }
   }
...
void old_load_from_string(mixed value, int recurse)
{
   ...
   if (!stringp(value)) { return; }

   value = restore_variable(value);

   if (value[0] != base_name(this_object()))
   {
      // If the saved fname is not a "legal" old name, then raise an error.
      // There are certain files that have moved, so we must deal appropriately with errors.
...
void load_from_string(mixed value, int recurse)
{
   ...
   if (data["#base_name#"] != base_name(this_object()))
   {
      // If the saved fname is not a "legal" old name, then raise an error.
      // There are certain files that have moved, so we must deal appropriately with errors.
      // use throw() so we don't get a trace; this is a normal exception in some situations.
