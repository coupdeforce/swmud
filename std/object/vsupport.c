...
mixed check_permission(string what)
{
   ...
   if (!who || who->allow(what)) { return 1; }
...
mixed direct_verb_rule(string verb,string rule,mixed args...)
{
   ...
   if (this_object()->is_exit())
   {
      ...
      if (!temp || stringp(temp)) { return temp; }
   }
...
mixed direct_get_obj(object ob)
{
   ...
   if (environment() == this_body()) { return "#You already have it!\n"; }
   ...
   if (environment(this_object()) != environment(this_body())) { return "#You cannot get that."; }

   if (!default_object_checks()) { return 0; }

   //:HOOK prevent_get
   //A yes/no/error hook called by direct_get_obj() if the standard conditions succeed
...
//Handle parser checks for get.  WRD in this instance should be a valid relation in obj2

//:FUNCTION direct_put_obj_wrd_obj
//Handle parser checks for "put OBJ WRD OBJ"
// mixed direct_put_obj_wrd_obj(object ob1, object ob2) { return check_permission("put"); }

//:FUNCTION direct_get_obj_with_obj
//Handle parser checks for "get OBJ with OBJ"
mixed direct_get_obj_with_obj(object ob1, object ob2) { return direct_get_obj(ob1); }
...
mixed direct_look_at_obj(object ob) { return default_object_checks(); }

//:FUNCTION direct_look_obj
//Whether or not an object can be looked at.  If there's no short description,
//or it's not visible to the looker, return 0.
mixed direct_look_obj(object ob) { return default_object_checks(); }

//:FUNCTION direct_look_for_obj
//Always allow "look for OBJ" to succeed by default.
mixed direct_look_for_obj(object ob) { return 1; }

//:FUNCTION direct_look_wrd_obj
//Set "look WRD OBJ" to fail by default.
mixed direct_look_wrd_obj(object ob) { return "#You can't look there"; }

//:FUNCTION direct_sell_obj
//Handle parser checks for "sell OBJ"
mixed direct_sell_obj(object ob) { return need_to_have(); }
...
mixed direct_give_wrd_str_to_liv() { return 1; }

//### This DEFINATELY shouldn't be here.  Should be in living.c
mixed indirect_give_wrd_str_to_liv() { return 1; }
...
mixed direct_sell_obj_to_liv(object ob, object liv, mixed foo) { return need_to_have(); }
...
mixed direct_drop_obj(object ob)
{
   ...
   if (environment() != this_body()) { return "#You don't have it!\n"; }

   //:HOOK prevent_drop
   //A yes/no/error hook called by direct_drop_obj() if the standard conditions succeed
...
/*
mixed direct_DONTFLIPTHISISBROKEN_obj(object ob)
{
   if (!default_object_checks()) { return 0; }
...
mixed direct_throw_obj(object ob1, object ob2) { return need_to_have(); }

//:FUNCTION direct_throw_obj_at_obj
//Handle parser checks for "throw OBJ at OBJ"
mixed direct_throw_obj_at_obj(object ob1, object ob2) { return need_to_have(); }

//:FUNCTION indirect_throw_obj_at_obj
//Handle parser checks for "throw OBJ at OBJ"
mixed indirect_throw_obj_at_obj(object ob1, object ob2) { return default_object_checks(); }
...
mixed direct_search_obj(object ob) { return default_object_checks(); }

//:FUNCTION direct_search_obj_for_obj
//Default
mixed direct_search_obj_for_obj(object ob1, object ob2) { return default_object_checks(); }

//:FUNCTION indirect_search_obj_for_obj
//Default
mixed indirect_search_obj_for_obj(object ob1, object ob2) { return default_object_checks(); }

//FUNCTION direct_search_for_str
//Default
mixed direct_search_for_str(string str) { return 1; }
...
//Default
mixed direct_search_for_str_in_obj_with_obj(string str, object ob1, object ob2) { return 1; }
