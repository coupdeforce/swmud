// Last edited by deforce on 01-25-2010
...
//
// Created by Deathblade on 11-05-1994. 

#pragma warnings

#define CHANNEL_FORMAT(chan) "%%^CHANNEL%%^[%s]%%^RESET%%^ %s\n"
...
// This mapping contains which channels should not be auto-purged (keys) and their flags (values)
...
protected nomask void create_channel(string channel_name)
{
   ...
   if (info[channel_name]) { error("Channel already exists.\n"); }
...
private void register_one(int add_hook, object listener, string channel_name)
{
   ...
   if ((ci->flags & CHANNEL_ADMIN_ONLY) && !adminp(this_user())
      && (SECURE_D->query_is_wizard(this_user()->query_userid()) < 3)
...
void kill_channel(string channel_name)
{
   class channel_info ci = info[channel_name];
...
private nomask string find_sender_name(string sender_name)
{
   ...
   if (this_body())
   {
      if (sender_name = capitalize(this_body()->living_query_name()))
      {
         return sender_name; 
      }
   }
...
nomask void deliver_string(string channel_name, string str)
{
   ...
   mixed array time_data = localtime(time());

   if (!ci || sizeof(ci->listeners) == 0) { return; }

   ci->history += ({ sprintf("%02i:%02i %s", time_data[2], time_data[1], str) });
...
{
   deliver_string(channel_name, sprintf(CHANNEL_FORMAT(channel_name),
      user_channel_name(channel_name), str));
}
...
nomask void deliver_raw_soul(string channel_name, mixed * data)
{
   ...
   ci->history += ({ sprintf("%02i:%02i %s", time_data[2], time_data[1], data[1][<1]) });
...
private nomask void deliver_data(string channel_name, string sender_name, string type, mixed data)
{
   class channel_info ci = info[channel_name];
...
nomask void deliver_tell(string channel_name, string sender_name, string message)
{
   ...
   deliver_string(channel_name, sprintf(CHANNEL_FORMAT(channel_name),
      user_channel_name(channel_name), sender_name + ": " + punctuate(message)));
}
...
{
   deliver_string(channel_name, sprintf(CHANNEL_FORMAT(channel_name),
      user_channel_name(channel_name), "(" + message + ")"));
}
...
void create()
{
   ...
   if (saved_listeners)
   {
      foreach (pair in saved_listeners)
      {
         ...
         if (ob) { register_one(0, ob, pair->channel_name); }
      }
   ...
   if (saved_hooks)
   {
      foreach (pair in saved_hooks)
      {
         ...
         if (ob) { register_one(1, ob, pair->channel_name); }
      }
   ...
   foreach (string channel_name, int flags in permanent_channels)
   {
      ...
      if (!info[channel_name]) { create_channel(channel_name); }
...
// remove()
// Write out all listeners and hooks that are blueprints.  We'll reset them at creation time.
void remove()
{
   string channel_name;
   class channel_info ci;
...
// query_flags()
// Return the flags for a particular channel.  0 is returned for unknown channels.
