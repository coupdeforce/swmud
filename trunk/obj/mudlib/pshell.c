...
{
   if (!shell_vars) { shell_vars = ([]); }

   if (name == "PROMPT") { this_user()->query_shell_ob()->set_prompt(value); }
...
private nomask string expand_one_argument(string arg)
{
   ...
   if ((strlen(arg) <= 1) || (arg[<1] != '*' )) { return arg; }

   expansion = complete_user(arg[0..<2]);

   if (stringp(expansion)) { return expansion; }
...
protected void execute_command(string original_input)
{
   ...
   if (is_file("/cmds/guild/jedi/" + argv[0] + ".c") && !this_body()->can_feel_force())
   {
      write("You are unable to feel the Force.\n");
      return;
   }

   winner = CMD_D->find_cmd_in_path(argv[0], ({ "/trans/cmds", "/cmds/player", "/cmds/race", "/cmds/guild/assassin", "/cmds/guild/bounty_hunter", "/cmds/guild/diplomat", "/cmds/guild/jedi", "/cmds/guild/mechanic", "/cmds/guild/mercenary", "/cmds/guild/merchant", "/cmds/guild/physician", "/cmds/guild/pilot", "/cmds/guild/slicer", "/cmds/guild/smuggler" }) + query_path());

   if (!arrayp(winner))
   {
      string channel_name;

      if (this_body()->do_game_command(original_input)) { return; }
      ...
      if (is_file(CMD_DIR_VERBS "/" + argv[0] + ".c")) { write(this_body()->nonsense()); }
      else { printf("I don't know the word: %s.\n", argv[0]); }

      return;
   }

   if (sizeof(argv) > 1) { argument = implode(argv[1..], " "); }
