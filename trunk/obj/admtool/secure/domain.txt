...
private nomask void create_domain(int no_domain_dir, string domain)
{
   ...
   if (write_error(SECURE_D->create_domain(domain))) { return; }

   write("Domain created.\n");

   if (!no_domain_dir)
   {
      string dirname = "/d/" + domain;
...
{
   SECURE_D->set_protection("/d/" + domain, 1, -1);

   // this checks for admins...
   if (write_error(SECURE_D->delete_domain(domain))) { return; }
...
private nomask void add_user_to_domain(int lord, string user, string domain)
{
   ...
   if (write_error(SECURE_D->add_domain_member(domain, user, lord))) { return; }
...
{
   // this checks for lord privs...
   if (write_error(SECURE_D->remove_domain_member(domain, user))) { return; }
