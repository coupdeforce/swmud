...
mixed can_remove_wrd(string wrd)
{
   ...
   foreach (object ob in deep_inventory(environment(this_body())))
   {
      ...
      {
         if (!brd) { brd = ob; }
         else { return "Please specify on which board you want to remove a note.\n"; }
      }
   }

   if (!brd) { return "There's nothing here to remove notes from.\n"; }

   if ((res = brd->direct_remove_wrd_from_obj(wrd, brd)) != 1) { return res; }
...
void do_remove_wrd(string wrd)
{
   ...
   foreach (object ob in deep_inventory(environment(this_body())))
   {
      if ((int)ob->is_bulletin_board()) { brd = ob; }
   }
...
void create()
{
   add_rules( ({ "WRD", "WRD from OBJ", "OBJ", "STR" }), ({ "take off" }));
}