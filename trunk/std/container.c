...
class relation_data
{
   ...
   int max_capacity;  // The maximum capacity that can be  maintained by that relation.
   int hidden;  // Whether or not the contents in  this relation can be seen or not
   int attached;  // Determines whether or not the contents of the relation will be added to the relations mass/size when calc'd
   mixed hidden_func;  // Function or int to be evaluated to determine whether or not the contents of the relation can be seen
   mapping create_on_reset;  // A mapping containing filenames, quantities and parameters needed for objects to be updated every reset() in the relation
   mapping create_unique_on_reset;  // Mapping similar to that above, but the entire game is checked for the existance of the object before cloning
}
...
private nomask int valid_relation(string relation)
{
   ...
   if (member_array(relation, keys(relations)) == -1) { return 0; }
...
int query_capacity(string relation)
{
   ...
   foreach(object ob in relations[relation]->contents)
   {
      ...
      if (ob->test_flag(F_WORN) || ob->test_flag(F_WIELDED))
      {
         cap += ob->query_weight() * 0.9;
      }
      else
      {
         cap += ob->query_weight();
...
mixed receive_object( object target, string relation )
{
   ...
   if ((m = (query_capacity(relation)) + target->query_weight()) > query_max_capacity(relation))
   {
      return MOVE_NO_ROOM;
   }
...
varargs mixed release_object(object target, int force)
{
   ...
   if (!target || force) { return 1; }

   relation = query_relation(target);

   if (!relation&&!force) { return 0; }
...
string look_in(string relation)
{
   ...
   if (!ex) { ex = call_hooks("prevent_look_all", HOOK_YES_NO_ERROR); }

   if (!ex) { ex = "That doesn't seem possible."; }

   if (stringp(ex)) { return ex; }
...
varargs void set_hide_contents(mixed hide, string relation)
{
   ...
   if (hide)
   {
      ...
      else
      {
         ...
         if (!valid_relation(relation))
         {
            if (!aliased_to)
            {
               error("Attempted to hide contents of a container with a nonexistant relation.");
               relation = aliased_to;
            }
   ...
   else 
   {
      ...
      if (!valid_relation(relation))
      {
         if (!aliased_to)
         {
            error("Attempted to unhide contents of a container with a nonexistant relation");
            relation = aliased_to;
         }
...
mixed array make_objects_if_needed()
{
   ...
   foreach (string relation, class relation_data object_data in relations)
   {
      ...
      foreach(string file, mixed parameters in object_data->create_on_reset)
      {
         ...
         // If the only parameter is an integer, it is the quantity of the object that needs to be in this relation
         if (intp(parameters)) { num = parameters; }
         else if (arrayp(parameters))
         {
            // Check the first argument for an integer value, if it is then it is the quantity fo the object to be in the relation
         ...
         while (sizeof(matches) < num)
         {
            ...
            if (ret != MOVE_OK) { error("Initial clone failed for '" + file +"': " + ret + "\n"); }
...
varargs string introduce_contents(string relation)
{
   ...
   if (!sizeof(all_inventory())) { return ""; }

   switch (relation)
   {
      case "in": return capitalize(the_short()) + " contains:\n";
      case "on": return "Sitting on " + the_short() +  " you see:\n";
      case "attached to": return "";
      case "holstered in": return "";
      default: return capitalize(relation) + " " + the_short() + " you see:\n";
   }
}
...
varargs string inventory_recurse(int depth, mixed avoid)
{
   ...
   if (!this_object()->is_living() && sizeof(obs))
   {
...
receive_inside_msg(string msg, object * exclude, int message_type, mixed other)
{
   ...
   {
      env->receive_inside_msg(msg, arrayp(exclude) ? exclude +  ({this_object()}) : ({this_object()}), message_type, other);
   }
}
...
varargs void receive_remote_msg(string msg, object array exclude, int message_type, mixed other)
{
...
void containee_light_changed(int adjustment)
{
   ...
   // if the containee is visible, then tweak our own light; this will propagate on up to the room
...
void resync_visibility()
{
   ...
   if (new_state == contained_light_added) { return; }
...
mixed indirect_verb_rule(string verb, string rule, mixed second, mixed third, mixed fourth)
{
   ...
   //    return 0;
   if (!is_visible()) { return "not visible"; }
