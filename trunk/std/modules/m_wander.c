...
// This module will allow monsters to wander in designated area,
// with specified time intervals between movements.
...
{
   if (member_array(area, wander_area) > -1)
   {
      wander_area -= ({ area });
   }
}
...
// Check here to see if there are other PC's in the room.
// If so, reset the count and continue moving.
void moving()
{
...
private void do_wander()
{
   ...
   if (!directions || !sizeof(directions))
   {
      return;
   }

   chosen_dir = directions[random(sizeof(directions))];
