...
private mixed base = this_object();  // The base directory to determine exit destinations from.  This can be either a string, object, or function pointer
...
private string array all_methods_matched(string method)
{
   ...
   // The first word of the method is expected to be the verb, the rest is a preposition, if it exists.  If it is only one word, that is the verb
...
private string array methods_matched(string method)
{
   ...
   // We only want to return those matches which are also methods on this object so eliminate those that aren't
...
//The argument can be a string (directory), object, or function pointer evaluating to one of these.
...
varargs void set_method(string method, mixed destination, mixed checks, mixed array exit_messages, mixed array enter_messages)
{
   ...
   {
      error(sprintf("Cannot set method '%s'.  Method is either too ambiguous or should be added to METHOD_D", method));
   }
...
//Set the checks for a method.  The checks can be either an int, function pointer, or string.
...
//the second argument is either a string or function pointer which will return a string.
...
string query_method_enter_message(string method)
{
   ...
   // We know that ret has to be an array at this point, select one element randomly
...
//second_part,mixed third)
{
   ...
   // The second argument is the rule used.  For our purposes it should always be WRD OBJ or OBJ, but lets make sure, just in case.
   ...
   // If there are more than one match, it is ambiguous and we can't determine which method to use
   if (sizeof(matches) > 1)
   {
...
int complex_exit_do_verb_rule(string verb, string rule, mixed args...)
{
   ...
   // For our purposes the rule should always be WRD OBJ or OBJ or no rule at all.  -- Check for no rule first.
   ...
   // We know now that there can be only one match, or it would not have passed the direct_verb_rule
