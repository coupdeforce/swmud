...
private string make_ansi_string(string file, string path)
{
...
private string make_l_flag_line(string basename, string fullname)
{
   if (is_dir(fullname))
   {
...
private mixed map_ls_arrays(function f, mixed arr1, mixed arr2)
{
   int i, s;
   mixed res;
...
int do_ls(mixed argv, mapping flags)
{
   ...
   // Changed because this is non-intuitive compared to the rest of the uses of colour -- Tigran.
   ...
   foreach(path in sort_array(keys(argv[0]),1))
   {
      ...
      {
         outarr = map_ls_arrays((: is_dir($1) ? $2 + "/" : info[$1] ? info[$1][2] ? $2 + "*" : $2 : $2 + "?":), fullpatharr, outarr);
      }
