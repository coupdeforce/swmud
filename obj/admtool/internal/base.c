...
protected void heading() {
    write("%^ADMTOOL_HEADING%^Administration Tool: " + module_name()
 + "
...
int maxlen = 0;      /* max length of "proto" field */
    ...
    foreach (class command_info comm in commands) {
        ...
        if (comm->who)
            desc = desc + repeat_string(" ", 65 - maxkeylen - maxlen - strlen(desc) - strlen(comm->who)) +
comm->who;
    ...
    foreach (class command_info comm in commands) {
        if (str == comm->key) {
            ...
            case 2:
                ...
                return;
            case 3:
                input_three_args(comm->args[0], comm->args[1], comm->args[2], comm->action, arg);
                return;
            default:
