...
// Pad the default array with empty strings if there aren't enough defaults provided to us.
...
varargs protected void cmd_list_aliases()
{
   foreach (string key in sort_array(keys(aliases), 1))
   {
      class alias this_alias = aliases[key];

      write(sprintf("%-12s : %-60s\n", key, this_alias->template));
   }
}
