...
private void stdin_grep(string pattern, mapping flags, string stdin)
{
   string array lines;
   int bits = 0;
   int i;
   ...
   if (flags["i"]) { pattern = insensitive_pattern(pattern); }
...
private void main(mixed argv, mapping flags, string stdin)
{
   ...
   if (!strlen(get_output())) { out("No matches found.\n"); }
}